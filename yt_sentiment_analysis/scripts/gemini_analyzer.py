...
class GeminiBrandAnalyzer:
    ...
    """
    def __init__(self, config_path='config_gemini.ini', env_path='.env'):
    ...
    def _load_configuration(self, config_path):
        ...
        # Analysis settings
        self.model_name = config.get('Analysis', 'model_name')
        self.prompt_template_path = config.get('Analysis', 'prompt_template_path')
        self.report_template_path = config.get('Analysis', 'report_template_path')
        ...
        # Make output paths relative to the project root, not the script's location
        self.output_dir = os.path.join(os.getcwd(), 'outputs', self.safe_brand_name)
        self.videos_csv_path = os.path.join(os.getcwd(), 'outputs', self.safe_brand_name, f"{self.safe_brand_name}_discovered_videos.csv")
        
        # If comments_csv_path is not specified in config, construct it dynamically
        if not self.comments_path:
            self.comments_path = os.path.join(os.getcwd(), 'outputs', self.safe_brand_name, f"{self.safe_brand_name}_raw_comments.csv")
        else:
            # Also make the config path relative to the project root
            self.comments_path = os.path.join(os.getcwd(), self.comments_path)
    ...
    def _generate_report(self, analysis_content):
        ...
        if self.report_format == 'html':
            ...
            if not os.path.exists(self.report_template_path):
                raise FileNotFoundError(f"Report template not found at '{self.report_template_path}'")
