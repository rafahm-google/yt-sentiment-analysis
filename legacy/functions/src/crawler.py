...
class YouTubeBrandCrawler:
    ...
    def _load_configuration_from_dict(self, config):
        ...
        self.search_modifiers = [mod.strip() for mod in config.get('search_modifiers', 'review, unboxing').split(',')]
        self.exclude_keywords = [key.strip().lower() for key in config.get('exclude_keywords', '').split(',')]
        self.min_view_count = int(config.get('min_view_count', 1000))
        self.sort_by = config.get('sort_by', 'relevance')
        self.max_results = int(config.get('max_results', 10))
        
        self.base_output_dir = config.get('base_output_dir', 'outputs')

        safe_brand_name = re.sub(r'\W+', '', self.search_terms.replace(' ', '_'))
        self.output_dir = os.path.join(self.base_output_dir, safe_brand_name)
