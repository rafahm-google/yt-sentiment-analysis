...
class CachedAnalysisPipeline:
    ...
    def _load_configuration(self):
        ...
        self.pro_prompt_path = config.get('Analysis', 'pro_prompt_template_path')
        self.flash_prompt_path = config.get('Analysis', 'flash_prompt_template_path')
        ...
        self.output_dir = os.path.join('outputs', self.safe_brand_name)
    ...
    def _process_batches(self, videos_df, comments_df):
        ...
        for i in tqdm(range(num_batches), desc="Processing Batches"):
            ...
            audio_files = [os.path.join(self.audio_dir, f"{vid}.mp3") for vid in batch_video_ids if os.path.exists(os.path.join(self.audio_dir, f"{vid}.mp3"))]

            summary = self._run_flash_analysis(batch_videos, batch_comments, audio_files)
    ...
    def _run_flash_analysis(self, videos, comments, audio_files):
        ...
        try:
            ...
            uploaded_files = [genai.upload_file(path=f) for f in audio_files]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        default="gemini_analysis_suite/config_gemini.ini",
